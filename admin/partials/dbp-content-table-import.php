...
<div class="dbp-content-table js-id-dbp-content" id="dbp_content_table" >
    ...
    <?php if (@$this->msg != "" && dbp_fn::get_request('action', '', 'string') != 'create-table-csv-data') : ?>
    ...
    <?php if (in_array($action, ['import-csv-file', 'execute-csv-data', 'create-table-csv-data', 'insert-csv-data']) ) : 
        ...
        <div id="first_block">
            <div class="dbp-alert-info">
                <?php _e('The csv file has been loaded, check if the data is correct.','database_press'); ?>
                <a href="<?php echo add_query_arg(['section'=>'table-import'],  admin_url("admin.php?page=database_press")); ?>"><?php _e('Upload a new file','database_press'); ?></a>
            </div>
                ...
                <div class="dbp-import-params-csv">
                    <form method="POST" action="<?php echo admin_url("admin.php?page=database_press&section=table-import"); ?>" enctype="multipart/form-data" >
                        ...
                        <?php if ((isset($allow_use_first_row) && $allow_use_first_row == true) || !isset($allow_use_first_row)) : ?>
                            <label><input type="checkbox" name="csv_first_row_as_headers" value="1" <?php echo ($csv_first_row_as_headers) ? ' checked="checked"' : '';?>> <?php _e('Use first row as Headers', 'database_press'); ?></label>
                        <?php else : ?>
                            <label><input type="checkbox"  disabled> <?php _e('Use first row as Headers', 'database_press'); ?></label>
                        <?php endif; ?>   
                        <input type="hidden" name="allow_use_first_row" value="<?php echo (@$allow_use_first_row) ? 1 : 0; ?>">
                        <input type="submit" value="<?php _e('Update Preview', 'database_press'); ?>" />
            ...
            <div class="dbp-choose-import-csv-action">
                <select id="dbp_import_select_action" onchange="dbp_toggle_action_import(this)">
                    <option value=""><?php _e('Choose action', 'database_press'); ?></option>
                    <option value="create_table" <?php echo (@$select_action == 'create_database') ? 'selected="selected"' : ''; ?>><?php _e('Create Table', 'database_press'); ?></option>
                    <option value="insert_records" <?php echo ($action == 'insert-csv-data' || @$select_action == 'insert_records') ? 'selected="selected"' : ''; ?>><?php _e('Insert/Update Records', 'database_press'); ?></option>
                </select>
                <?php dbp_fn::echo_html_icon_help('database_press-table-import','choose_action'); ?>
            </div>
            <?php if (dbp_fn::get_request('action', '', 'string') == 'create-table-csv-data') : ?>
            ...
            <div class="js-dbp-import-content-toggle" id="dbp_content_create_table" style="display:none">
                <form method="POST" action="<?php echo admin_url("admin.php?page=database_press&section=table-import"); ?>" id="dbp_create_table">
                    ...
                    <div id="dbp_create_table" class="dbp-import-content-create-table" style="<?php echo (dbp_fn::get_request('action', '', 'string') == 'create-table-csv-data') ? '' : '' ; ?>">
                        ...
                        <table class="wp-list-table widefat striped dbp-table-view-list js-dragable-table">
                            <thead>
                                <th><?php _e('Order','database_press'); ?></th>
                                <th><?php _e('Table name','database_press'); ?></th>
                                <th><?php _e('Preset','database_press'); ?></th>
                                <th><?php _e('Import from CSV column','database_press'); ?></th>
                                <th><?php _e('Action','database_press'); ?></th>
                            ...
                            <tr class="js-clore-master">
                                ...
                                <td>
                                    <div class="button" onClick="dbp_import_csv_create_table_delete_row(this);"><?php _e('Delete Row' , 'database_press'); ?></div>
                                </td>
                            </tr>
                            <?php
                            ...
                            <tr>
                                <td colspan="5">
                                    <div onclick="dbp_create_table_add_row(this, '<?php echo @$max_row_allowed; ?>')" class="button"><?php _e('Add row', 'database_press'); ?></div>
                                </td>
                            </tr>
                        </table>
                    </div>
                    <div class="dbp-box-create-table-box">
                        <div id="dbp_content_button_create_form" >
                            <input type="submit" class="dbp-submit" value="<?php _e('Create Table', 'database_press'); ?>" /> 
        ...
        <div class="js-dbp-import-content-toggle" id="dbp_content_insert_records" style="display:none">
            <form id="dbp_import_csv_data_config" method="POST" action="<?php echo admin_url("admin.php?page=database_press&section=table-import"); ?>" >
                ...
                <div class="dbp-msg-unique-table-id-explane"><?php _e('Select one or more tables in which to insert data.<br>On each field choose which column of the csv to insert.<br>If you want to insert data on multiple tables, for example post and postmeta: First select the post table and associate the fields of your csv. At the bottom of the table you will find a string. This refers to the id of the records that will be created or updated.<br>Add the postmeta table and on the post_id field select the code in square brackets.<br>','database_press'); ?></div>
                ...
                <div class="dbp-import-content-clone-block">
                    <div class="js-insert-fields-content-clone dbp-insert-fields-content">
                        <div class="dbp-import-params-csv">
                            <?php dbp_fn::html_select(array_merge([''=>__('Select table', 'database_press')],  $this->table_list['tables']), true, 'class="js-select-tables-import jsonchange-select-tables-import-clone"', @$current_table); ?>
                            ...
                            <?php endif ; ?> 
                        <div class="js-immport-choose-table-remove-btn button" style="display:none"><?php _e('Delete','database_press'); ?></div>
                        </div>
                        <div class="js-content-table-fields"></div>
                        <div class="js-msg-yes-pri-key dbp-msg-yes-no-pri-key" style="display:none">* <?php _e('Records with the same primary key will be updated.','database_press'); ?></div>
                        <div class="js-msg-no-pri-key dbp-msg-yes-no-pri-key"  style="display:none">* <?php _e('All records will be inserted as new.','database_press'); ?></div>
                        <div class="dbp-msg-unique-table-id" >* <?php _e('If you want to insert data into multiple tables, you can reference the newly created or updated record ID using the following code:','database_press'); ?> <span class="js-unique-code"></span></div>
                    </div>
                </div>
            ...
            <div class="dbp-import-content-all-block">
                <div id="dbp_import_csv_btns">
                    <div class="button" onclick="dbp_csv_test_import()"><?php _e('Test the Import', 'database_press'); ?></div>
                    &nbsp; <div class="dbp-submit" onclick="dbp_csv_exec_import(0,0,0,0)"><?php _e("I'm feeling lucky! Import the data", 'database_press'); ?></div>
    ...
    <?php else: // in_array($action, ['import-csv-file', 'execute-csv-data', 'create-table-csv-data', 'insert-csv-data']
        ...
        * Forse è meglio poter caricare zip e basta!
        ...
        ?>
        <div class="dbp-content-margin">
            <h2 class="dbp-h2"><?php _e('File to import', 'database_press'); ?></h2>
            <p class="dbp-p"><?php 
            _e('Upload an <b>sql</b> file with the queries to run', 'database_press'); 
            ...
            <form method="POST" action="<?php echo admin_url("admin.php?page=database_press&section=table-import"); ?>" enctype="multipart/form-data" >
                ...
                <input type="submit" class="dbp-submit" value="<?php _e('Execute import', 'database_press'); ?>" />
            </form>
            <hr>
            <p class="dbp-p"><?php 
            _e('Upload a <b>csv</b> file and go ahead to configure the import', 'database_press'); 
            ...
            <form method="POST" action="<?php echo admin_url("admin.php?page=database_press&section=table-import"); ?>" enctype="multipart/form-data" >
                ...
                <input type="hidden" name="table" value="<?php echo @$import_table; ?>" />
                <input id="fileupload" name="sql_file" type="file" />
                <input type="submit" class="dbp-submit" value="<?php _e('Go ahead', 'database_press'); ?>" />
            </form>
            ...
            if ($max > 0) {
                ?>  <hr> <br><?php 
                printf(__("max upload files <b>%s</b>", 'database_press'), $max); 
