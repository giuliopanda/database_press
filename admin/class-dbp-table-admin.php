<?php
/**
 * Il controller dell'amministrazione che gestisce tutte le chiamate alle pagine del plugin (quindi quando stai dentro page=database_press).
...
class database_press_admin 
{
		...
		switch ($section) {
			...
			case 'table-import' :
				...
				if ($action =='import-sql-file') {
					...
					$import_table = dbp_fn::get_request('table', '');
	...
	private function table_structure() {
		...
		if ($action == 'edit-index') {
			wp_enqueue_script( 'database-press-import-js', plugin_dir_url( __FILE__ ) . 'js/database-press-structure.js',[],rand());
			wp_enqueue_script( 'jquery-ui-sortable' );
			...
			wp_enqueue_script( 'jquery-ui-sortable' );
			wp_enqueue_script( 'database-press-import-js', plugin_dir_url( __FILE__ ) . 'js/database-press-structure.js',[],rand());
			wp_enqueue_script( 'database-press-alter-table-js', plugin_dir_url( __FILE__ ) . 'js/database-press-alter-table.js',[],rand());
			...
			if ($action == 'save_metadata') {
				...
				dbp_fn::update_dbp_option_table_status($table, $_REQUEST['options']['status'], $_REQUEST['options']['description']);
			}
			if ($action == 'save_index') {
				if (isset($_REQUEST['index']['columns'])) {
					...
					if ($table_model->alter_index($_REQUEST['index']['columns'], $_REQUEST['original_name'], $name, $_REQUEST['original_index'], $_REQUEST['index']['type'])) {
						$msg =__('Altered Index success', 'database_press');
					} else {
						$msg_error = $table_model->last_error;
					}
				} else {
					$msg_error = __('You must select at least one column', 'database_press');
				}
			}
			if ($action == 'delete-index') {
				if ($table_model->delete_index( $_REQUEST['dbp_id'])) {
					$msg =__('Delete Index success', 'database_press');
				} else {
					$msg_error = __('Error Delete Index success', 'database_press');
				}
			}
			...
			if ($table_model->error_primary) {
				$msg_error = __('This system works better with tables that have only one field set as the autoincrement primary key.','database_press');
				$msg_error .= '<br><br>';
				if ($table_options['status'] != 'DRAFT') {
					$msg_error .= __('<b>To solve the problem follow the instructions:</b><br>
					<p>1. Click on Edit status and put the table in DRAFT MODE</p><p>2. Copy and run the queries</p>','database_press');
				} else {
					$msg_error .= __('<b>Copy and run the queries to correct the problem.</b>','database_press');
				}
				if (count($old_primaries) == 1 &&  $is_old_primaries_type_numeric ) {
					$msg_error .= sprintf(__('<p style="background:#F2F2F2; border:1px solid #EEE; padding:.5rem">ALTER TABLE `%s` MODIFY %s INT NOT NULL AUTO_INCREMENT;<br></p>', 'database_press'), $table, implode(", ", $old_primaries));
					...
					ALTER TABLE `%s` ADD dbp_id BIGINT AUTO_INCREMENT PRIMARY KEY;<br></p>','database_press'), $table, $table, implode(", ", $old_primaries), $table);
				} else {
					$msg_error .= sprintf(__('<p style="background:#F2F2F2; border:1px solid #EEE; padding:.5rem">ALTER TABLE `%s` ADD dbp_id BIGINT AUTO_INCREMENT PRIMARY KEY;<br></p>','database_press'), $table);
				}
			}
	...
	private function table_browse() {
		...
		wp_register_script( 'dbp_database_press_js',  plugin_dir_url( __FILE__ ) . 'js/database-press.js',false,rand());
		...
		wp_enqueue_script( 'database-sql-editor-js', plugin_dir_url( __FILE__ ) . 'js/database-sql-editor.js',[],rand());
		wp_enqueue_script( 'database-press-js-multiqueries', plugin_dir_url( __FILE__ ) . 'js/database-press-multiqueries.js',[],rand());
		
		$msg =  $msg_error = '';
		if (isset($_COOKIE['dbp_msg'])) {
			$msg = stripslashes($_COOKIE['dbp_msg']);
		}
		if (isset($_COOKIE['dbp_error'])) {
			$msg_error = stripslashes($_COOKIE['dbp_error']);
		}	
		$temporaly_files = new Dbp_temporaly_files();
		// $dbp = new Dbp_fn();
        $section =  dbp_fn::get_request('section', 'home');
		...
		if ($action == "delete_rows" && isset($_REQUEST["remove_ids"]) && is_array($_REQUEST["remove_ids"])) {
			$result_delete = dbp_fn::delete_rows($_REQUEST["remove_ids"], $_REQUEST['custom_query']);
		...
		$custom_query = dbp_fn::get_request('custom_query', '');
		...
		if (strlen($custom_query) > 150000) {
			...
			if ($msg_error == "") {
				if (is_countable($mysqli->error_list) && count($mysqli->error_list) > 0) {
					...
					$msg = __('The queries were performed successfully.', 'database_press');
				}
			} 
		...
		$table_model->prepare(dbp_fn::get_request('custom_query', ''));
		...
		if ($table_model->sql_type() == "multiqueries") {
			...
			if ( count($table_model->get_pirmaries()) == 0 && count($table_model->get_query_tables()) > 0 && $msg_error == '' && $table_model->sql_type() == "select") {
				$msg_error = __('This system works better with tables that have only one field set as the autoincrement primary key.','database_press');
				$msg_error .= '<br>'.__('Most of the features have been disabled.','database_press').'<br>';
				$msg_error .= '<br>'.__('If you can modify the table, go to the structure tab and follow the proposed instructions.','database_press').'<br>';
				if ($table_model->table_status() == "DRAFT")  {
					$msg_error .= '<b>'.__('If you can alter the table, go to Structure and follow the instructions.','database_press').'</b>';
				}
			}
			if ($table_model->table_status() == 'CLOSE' && $msg == '' && $msg_error == '') {
				$msg = __('The table can no longer be modified because it is in the "CLOSE" state.', 'database_press');
			}
		...
		if (!isset($_FILES['sql_file']['tmp_name']) || $_FILES['sql_file']['tmp_name'] == "") {
			...
			if ($this->last_error == "") {
				if (is_countable($mysqli->error_list) && count($mysqli->error_list) > 0) {
					...
					$this->msg = sprintf(__('%s queries executed successfully.', 'database_press'), $count_query );
				}
			} 
	...
	function import_csv_file() {
		wp_enqueue_script( 'database-press-import-js', plugin_dir_url( __FILE__ ) . 'js/database-press-import.js',[],rand());
		wp_enqueue_script( 'jquery-ui-sortable' );
		...
		if ($csv_filename == "") {
			...
			if ($csv_structure == false) {
				$this->last_error = __("It doesn't look like a valid csv", 'database_press');
				$action = '';
			}
		}
		$render_content = "/dbp-content-table-import.php";
	...
	function execute_csv_data() {
		...
		$csv_filename = dbp_fn::get_request('csv_temporaly_filename');
		$csv_delimiter = dbp_fn::get_request('csv_delimiter');
		$allow_use_first_row = dbp_fn::get_request('allow_use_first_row', 1);
		$model_structure = new Dbp_model_structure();
		$name_of_file = $model_structure->change_unique_table_name(dbp_fn::get_request('csv_name_of_file'));

		$csv_first_row_as_headers = dbp_fn::get_request('csv_first_row_as_headers', false, 'boolean');
	...
	function create_table_csv_data() {
		...
		$model_structure = new Dbp_model_structure($_REQUEST['csv_name_of_file']);
		...
		if ($execute_query) {
			...
			if ($model_structure->last_error != "") {
				...
				if (is_wp_error($result) || !empty($wpdb->last_error)) {
					...
					$this->msg = __('Table <b>'. $model_structure->get_table_name(). '</b> created','database_press');
	...
	function filter_render_sql_btns($btns) {
		...
		'<div id="dbp-bnt-go-query"  class="dbp-submit dbp-btn-show-sql-edit"  onclick="dbp_submit_custom_query()">'. __('Go','database_press').'</div>'],$btns) ;
	...
	function home_render_sql_btns($btns) {
		unset($btns['cancel']);	
		$btns = array_merge(['go_custom' =>
		'<div id="dbp-bnt-go-query"  class="dbp-submit dbp-btn-show-sql-edit"  onclick="jQuery(\'#table_sql_home\').submit()">'. __('Go','database_press').'</div>'],$btns) ;
		return $btns;
	}
	...
	function browse_table_filter_render_sql_btns($btns) {
		...
		<div id="dbp-bnt-save-query" class="button js-show-only-select-query"  onclick="dbp_show_save_sql_query()">'. __('Create list from query','database_press').'</div></div>'], $btns) ;
		$btns = array_merge(['go_custom' =>
		'<div id="dbp-bnt-go-query" class="dbp-submit" onclick="dbp_submit_table_filter(\'custom_query\')">'. __('Go','database_press').'</div>'], $btns) ;
		$btns['search'] =  '<div  class="button js-show-only-select-query  dbp-btn-disabled js-btn-disabled" onclick="dbp_search_sql()">'. __('Search','database_press').'</div>';
	
		/*
		<div class="tablenav top dbp-tablenav-top">   
                <input type="search"  name="search" value="<?php echo dbp_fn::get_request('search'); ?>">
