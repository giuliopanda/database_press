 ...
 */
function dbt_multiqueries_ajax(filename) {
    ...
    jQuery('#dbt_count_queries_executed').css('display','inline-block');

    jQuery.ajax({
        type : "post",
        dataType : "json",
        url : ajaxurl,
        data: { action: "dbt_multiqueries_ajax", filename: filename, ignore_errors:document.getElementById('dbt_ignore_errors').checked }
    }).done(function (ris) {
        jQuery('#dbt_count_queries_executed').text(ris.executed_queries);
        if (ris.executed_queries < ris.total_queries) {
            if (ris.last_error != '') {
                ...
                dbt_multiqueries_ajax(ris.filename);
            }
        } else {
            // FINE!
...
function dbt_multiqueries_cancel() {
