jQuery(document).ready(function ($) {
    ...
    jQuery('#dbt_import_select_action').change();

    jQuery('.jsonchange-select-tables-import-clone').change(function() {
        dtf_import_add_table_fields_for_insert(this);
    })

    jQuery('.js-unique-primary').change(function() {
        dbt_unique_primary(this);
    });
 ...
 */
function dbt_create_table_add_row(el, max_allow) {
...
function dbt_import_csv_create_table_delete_row(el) {
 ...
 * @param DOM el 
 */
function dbt_toggle_action_import(el) {
    let partial_id = jQuery(el).val();
    jQuery('.js-dbt-import-content-toggle').css('display','none');
    jQuery('#dbt_content_'+partial_id).css('display','block');
   
    jQuery('#dbt_result_import_box').css('display','none');
    jQuery("#dbt_content_table").animate({ scrollTop: jQuery('#dbt_import_select_action').offset().top }, 100);
}
 ...
 */
function dtf_import_add_table_fields_for_insert(el) {
    console.log ('dtf_import_add_table_fields_for_insert');
    ...
    jQuery(el).val('');
    dbt_load_table_structure(id, select_val);
 ...
 * @param String table 
 */  
function dbt_load_table_structure(elid, table) {
    console.log ('dbt_load_table_structure');
   // console.log ("dbt_load_table_structure id: "+elid+" table: "+table );
    ...
    jQuery.ajax({
        ...
        data : {
            ...
            action:'dbt_import_csv_table_structure', 
            elid:elid, 
            table:table, 
            ...
            if ($block) {
                if (response.result == "ok") {
                    ...
                    jQuery('.js-insert-fields-content').each(function() {
                        if (jQuery(this).data('dbt_uniqueid')) {
                            add_select_option($block, jQuery(this).data('dbt_uniqueid'));
                        }
                    });                  
                    ...
                    $block.data('dbt_uniqueid',response.unique);
...
function add_custom_select_option() {
    ...
    jQuery('.js-fields-choosen').each(function() {
        $e = jQuery(this);
        if ($e.data('jsdbt_custom') !== 1) {
            $e.data('jsdbt_custom', 1);
            $input = jQuery('<textarea rows="1" class="dbt-textarea-pinacode js-show-pinacode-link"></textarea>');
            $input.prop('name', $e.prop('name'));
            $input.css('display', 'none');
            $e.prop('name',"");
            jQuery(this).after($input);
           
            $e.data('jsdbt_input',$input);
            if ($e.val() != "") {
                if ($e.val().indexOf(" ") > -1) {
            ...
            $e.change(function() {
                $input = jQuery(this).data('jsdbt_input');
                if (jQuery(this).val() == "__[custom]__") {
                    jQuery(this).css('display','none');
                    $input.css('display', 'inline-block');
                    dbt_show_pinacode_link($input);
                    $btn = jQuery('<span class="dashicons dashicons-list-view dbt-input-button-margin"></span>');
 ...
 */
function dbt_csv_test_import() {
    console.log ('dbt_csv_test_import');
    jQuery('#dbt_result_test_import_csv').empty().css('display','none');
    jQuery('#dbt_import_csv_exec_import').css('display','none');
    jQuery('#dbt_result_import_box').css('display','block');
    jQuery('#dbt_import_csv_alert').removeClass('dtf-alert-sql-error dtf-alert-info').addClass('dtf-alert-gray').css('display','block');
    jQuery('#dbt_import_csv_alert').html('Testing data');
    var data = jQuery('#dbt_import_csv_data_config').serializeArray() ;
    data.push({name: 'action', value:'dbt_test_import_csv_data'});
    jQuery.ajax({
        type : "post",
        dataType : "json",
        url : ajaxurl,
        data : data,
        success: function(response) {
            jQuery('#dbt_result_test_import_csv').css('display','block');
            jQuery('#dbt_import_csv_alert').css('display','none');
            if (response.result == 'ok') {
                for (x in response.html) {
                    jQuery('#dbt_result_test_import_csv').append(response.html[x].html);
                }
            } else if (response.msg) {
                jQuery('#dbt_import_csv_alert').removeClass('dtf-alert-info dtf-alert-gray').addClass('dtf-alert-sql-error').css('display','block');
                jQuery('#dbt_import_csv_alert').html(response.msg);
            } else {
                jQuery('#dbt_import_csv_alert').removeClass('dtf-alert-info dtf-alert-gray').addClass('dtf-alert-sql-error').css('display','block');
                jQuery('#dbt_import_csv_alert').html('There was an error testing the data');
            }
        }
    });
}
 ...
 */
 function dbt_csv_exec_import(insert, update, errors, total_row_executed) {
    jQuery('#dbt_result_test_import_csv').empty().css('display','none');
    ...
    jQuery ('#dbt_import_csv_data_config').css('display','none');
    jQuery ('#first_block').css('display','none');
    jQuery ('#dbt_import_csv_btns').css('display','none');
    
    jQuery('#dbt_result_import_box').css('display','block');
    jQuery('#dbt_import_csv_alert').removeClass('dtf-alert-sql-error dtf-alert-info').addClass('dtf-alert-gray').css('display','block');
    jQuery('#dbt_import_csv_alert').html('Importing data');
    var data = jQuery('#dbt_import_csv_data_config').serializeArray() ;
    data.push({name: 'action', value:'dbt_import_csv_data'});
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            if (response.result == 'ok') {
                jQuery('#dbt_import_csv_alert').html('I am loading data, please do not close the browser window or change the page');
                jQuery('#dbt_import_csv_exec_import').css('display','table');
                jQuery('#dbt_result_import_csv_total_row').text(response.total_row);
                jQuery('#dbt_result_import_csv_errors').text(response.errors);
                jQuery('#dbt_result_import_csv_insert').text(response.insert);
                jQuery('#dbt_result_import_csv_update').text(response.update); 
                if (response.break == 1) {
                    // devo richiamare nuovamente la query perché stava andando in timeout
                    dbt_csv_exec_import(response.insert, response.update, response.errors, response.total_row_executed);
                } else {
                    jQuery('#dbt_import_csv_alert').removeClass('dtf-alert-sql-error dtf-alert-gray').addClass('dtf-alert-info').css('display','block');
                    jQuery('#dbt_import_csv_alert').html('Import complete');
                    jQuery('#btn_csv_download').css('display','block');
                }
              
            } else {
                jQuery('#dbt_import_csv_alert').removeClass('dtf-alert-info dtf-alert-gray').addClass('dtf-alert-sql-error').css('display','block');
                jQuery('#dbt_import_csv_alert').html('There was an error importing the data');
            }
        }
    });
}