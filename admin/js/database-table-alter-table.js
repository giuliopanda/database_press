 ...
 */
 jQuery(document).ready(function () {
     ...
     */
    jQuery('#dbt_partial_name_table').on('keyup', function(e) {
        table_real_name();
    })

    jQuery('#dbt_table_use_prefix').change(function(e) {
        table_real_name();
    })
 ...
 */
 function dbt_alter_table_delete_row(el) {
 ...
 */
 function dbt_alter_add_row(el, max_allow) {
    ...
    dbt_preselect($tr2.find('.js-field-preselect').get(0));
 }
...
function table_real_name() {
    let new_name = "";
    if (jQuery('#dbt_table_use_prefix').is(':checked')) {
        new_name = jQuery('#dbt_table_use_prefix').val();
        jQuery('#dbt_wp_prefix').css('visibility','');
    } else {
        jQuery('#dbt_wp_prefix').css('visibility','hidden');
    }
    new_name += jQuery('#dbt_partial_name_table').val();
    jQuery('#dbt_structure_table_name').val(new_name);
}
...
function dbt_check_primary() {
    ...
    if (count_primary == 1) {
        ...
        jQuery('#dbt_content_button_create_form_msg_no_primary').css('display','block');
        jQuery('#dbt_content_button_create_form').css('display','none');
        jQuery('#dbt_execute_query_command').css('display','none');
    }
}

function dbt_preselect(el) {
