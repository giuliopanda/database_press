...
class  Dbp_fn {
    ...
    static function require_init() {
        ...
        require_once(dbp_DIR . "includes/dbp-render-list.php");
    ...
    static function get_request_limit($path, $default, $max = 1000) {
        $limit = dbp_fn::get_request($path, $default, 'absint');
        if ($limit < 1) {
            return $max;
    ...
    static function get_request_sort($path, $default_field = "", $default_order = "") {
        $sort_array = dbp_fn::get_request($path, true); 
        return $sort_array;
    }
    ...
    static function get_request_filter($path) {
        $filter_array = dbp_fn::get_request($path); 
        $result = [];
    ...
    static function sanitaze_request($path, $default = "") {
        $value = dbp_fn::get_request($path, $default);
  ...
  'DOUBLE' => 'NUMBER', 'LONGLONG' => 'NUMBER', 'INT24' => 'NUMBER', 'NEWDECIMAL' => 'NUMBER', 'TIMESTAMP'=>'DATE', 'DATE'=>'DATE', 'TIME'=>'DATE', 'YEAR'=>'DATE', 'DATETIME'=>'DATE','NEWDATE'=>'DATE', 'ENUM'=>'VARCHAR', 'SET'=>'VARCHAR', 'BLOB' => 'TEXT', 'TINY_BLOB'=>'TEXT', 'MEDIUM_BLOB'=>'TEXT', 'LONG_BLOB'=>'TEXT', 'VAR_STRING'=>'VARCHAR', 'STRING' => 'VARCHAR', 'INTERVAL' => 'DATE', 'BIT'=>'NUMBER' ];
            ...
            if (is_array($option_value)) {
                ?> <optgroup label="<?php echo esc_attr($option_key); ?>"><?php
                    foreach ($option_value as $option2_key => $option2_value) {
                        ...
                        $option2_value = __($option2_value, 'database_press');
                ...
                $option_value = __($option_value, 'database_press');
    ...
    static function get_array_for_select_in_drowdown_filter($symple_type) {
        $html_select_array = [
            ...
            'LIKE'  => __('Search Text (%LIKE%)',  'database_press'),
            'NOT LIKE'  => __('Exclude Text (NOT LIKE)',  'database_press'),
            'BETWEEN'  => __('Between Values (BETWEEN)',  'database_press'),
            'NOT BETWEEN'  => __('Values outside the range (NOT BETWEEN)',  'database_press'),
            'NULL' => __('Empties',  'database_press'),
            'NOT NULL'  => __('Not Empties',  'database_press') ];
        $def_op = "=";
        switch ($symple_type) {
            case "DATE":
                $def_op = "BETWEEN";
            ...
            case "VARCHAR":
                $html_select_array = [
                    '=' => __('Equals (=)',  'database_press'),
                    '!=' => __('Does Not Equal (!=)',  'database_press'),
                    '>cast'  => __('Greater Than (>)',  'database_press'),
                    '>=cast'  => __('Greater or Equal To (>=)',  'database_press'),
                    '<cast'  => __('Less Than (<)',  'database_press'),
                    '<=cast'  => __('Less or Equal To (<=)',  'database_press'),
                    'LIKE'  => __('Search Text (%LIKE%)',  'database_press'),
                    'NOT LIKE'  => __('Exclude Text  (NOT LIKE)',  'database_press'),
                    'NULL' => __('Empties',  'database_press'),
                    'NOT NULL'  => __('Not Empties',  'database_press') ];
    ...
    static function add_request_filter_to_model(&$table_model,  $max_show_items = 100) {
        $action_query = dbp_fn::get_request('action_query','');
        $table_filter = true;
    ...
    static function html_query_export($text, $query ) {
        ...
        <form method="post" action="<?php echo admin_url("admin-post.php"); ?>" class="dbp-form-single-query">
            ...
            <button type="submit" class="dbp-submit-style-link" ><?php _e($text, 'database_press'); ?></button> 
        </form>
        <?php
    }
   /**
	...
	static function execute_multiqueries($filename, $ignore_error = false) {
		...
		if ($temporaly->last_error) {
			...
			$last_error = __('No temp data found','database_press');
		} else {
			$count_queries = 0;
			while ($queries) {
    ...
    static function get_max_input_vars($input_used = 0) {
        ...
        if ($input_used > 0) {
            if ($input_used > $max_input) {
                return sprintf(__('The maximum number of input_var (max_input_vars) allowed by php.ini is %s But to manage this table I need at least %s', 'database_press'),$max_input, $input_used);
            } else {
                return "";
            }
        }
        ...
        if ($table_status != 'CLOSE') {
            foreach ($table_model->items as $key => $_) {
                ...
                if (isset($post_content["form_table"])) {
                    $edit = false;
                  //  var_dump($post_content["form_table"]);
                ...
                if ($dbp_id > 0) {
                    $btns = apply_filters('dbp_items_add_action',  $btns, $dbp_id, $count_unique_id);
                }
    ...
    static function prepare_delete_rows($rids, $request_sql = "", $dbp_id = 0) {
                    ...
                    foreach ($table_options as $table_option) {
                        if ($column == $table_option->table . "." . $table_option->pri_orgname) {
                            ...
                            if ($option['status'] != "CLOSE") {
                                ...
                                $errors[] = sprintf(__('Records in the "%s" table cannot be removed because they are in a closed state. If you want to be able to remove the data, change the status from the table structure to "published"', 'database_press'), $table_option->orgtable);
                            }
                        }
                    }
        ...
        if ($dbp_id > 0) {
            $show_msg = __('Are you sure you want to remove the <b>%s</b> selected records?', 'database_press');
        }
	...
	static function dbp_delete_from_sql($sql, $table_choose) {
		...
		dbp_fn::require_init();
		$table_model = new Dbp_model();
        $error = __("There was an unexpected problem", 'database_press'); 
						...
						if ($new_query != "" && $new_id_schema->table == $table_choose) {
                            ...
                            if ($option['status'] != "CLOSE") {
                                ...
                                if (!$wpdb->query($sql_to_del)) {
                                    $error = __(sprintf('Query "%s" returned an error', $sql_to_del), 'database_press');
                                } 
                            } else {
                                $error = __(sprintf('Records in the "%s" table cannot be removed because they are in a closed state. If you want to be able to remove the data, change the status from the table structure to "published"', $th['schema']->orgtable), 'database_press');
                            }
							break;
						}
					}
    ...
    */
    ...
    static function echo_html_icon_help($file, $anchor) {
        ?><span class="dashicons dashicons-info dbp-help-info-icon" onclick="anchor_help('<?php echo esc_attr($file); ?>','<?php echo esc_attr($anchor); ?>')" title="<?php _e('click_me', 'database_press'); ?>"></span><?php
    }
    ...
    static function get_latest_list() {
        ...
        if (is_countable($query_lists)) {
            foreach ($query_lists as $ql) {
                $ql->post_content = dbp_functions_list::convert_post_content_to_list_params($ql->post_content);
                $lists[$ql->ID] = $ql;
            }
        }
        return $lists;
