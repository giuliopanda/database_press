...
class Dbt_html_table_frontend {
	...
	/**
	 * Verifico se devo generare un uniq_id random oppure se gli viene passato da 
	 */
	function set_uniqid($uniq_id = "") {
		if ($uniq_id != "") {
	 ...
	 * @return String
	 */
	public function template_render_old($table_model, $ajax = false) {
		ob_start();
...
/**
	...
	public function template_render($table_model, $ajax = false) {
		$parts = $this->template_render_parts($table_model);
		ob_start();
		$search = new Dbt_search_form();
		if ($table_model->last_error !== false) {
			?><div class="dbt-alert-error"><?php _e('The table is currently unavailable','database_tables'); ?></div><?php
		...
		?>
		<div class="dbt-max-large-table<?php echo $add_class_size; ?>"><?php
			echo $parts['search_open'];
			if (in_array($this->get_frontend_view('table_pagination_position'), ["up",'both']) ) {
				echo $parts['pagination'];
			}
			if (($this->no_result == '' || empty($this->no_result)) || count($table_model->items) > 1) {
				echo $parts['table'];
			}
	...
	/**
	 * Ritornano le varie parti della tabella separate quindi senza definire dove devono essere messe
	 * @param Class $table_model
	 * @param Boolean ajax 
	 * @return Array
	 */
	public function template_render_parts($table_model) {
		/** @var Array $html_parts le varie parti html che compongono una lista */
		// TODO $uniqid_div da gestire
		$uniqid_div = $this->uniqid_div;
		$html_parts = [];
		$search = new Dbt_search_form();
		if ($table_model->last_error !== false) {
		...
		$html_parts['search_open'] = ob_get_clean();
		ob_start();
		$this->pagination($table_model);
		$html_parts['pagination'] = ob_get_clean();
		ob_start();
		if (($this->no_result == '' || empty($this->no_result)) || count($table_model->items) > 1) {
			?><?php
			$this->render($table_model->items);
		...
		$html_parts['table'] = ob_get_clean();

		ob_start();
		if ($this->get_frontend_view('table_update','get') != 'get') {
			$search->close_form(false);
		}
		$html_parts['search_close'] = ob_get_clean();
