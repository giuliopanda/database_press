...
class  Dbp_init_shortcode {
    /**
     * Imposta gli shortcode
     */
    public function __construct() {
        add_shortcode('dbp_list', [$this, 'dbp_list']);
        add_shortcode('dbp_tmpl_engine', [$this, 'dbp_tmpl_engine']);
    }

    /**
     * Mostra una lista frontend
     */
    public function dbp_list($attrs = []) {
        if (is_admin()) return;
        dbp_fn::require_init();
        $prefix = "";
        ...
        if ($attrs['id'] > 0) {
            $post_id = $attrs['id'];
            unset($attrs['id']);
        	return Dbp::get_list($post_id, false, $attrs, $prefix);
        }
    }
     ...
     */
    public function dbp_tmpl_engine($attrs = [], $content = "") {
        if (is_admin()) return;
        dbp_fn::require_init();
