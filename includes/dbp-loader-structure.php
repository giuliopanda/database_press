...
class  Dbp_loader_structure {
	...
	public function dbp_update_table_structure_test() {
		dbp_fn::require_init();
		$result = ['result'=>'ok'];
		$model_structure = new Dbp_model_structure($_REQUEST['table']);
		...
		$table_temp = dbp_fn_structure::create_temporaly_table_from($_REQUEST['table']);
		...
		foreach ($req_update['field_original_name'] as $key=>$field_update) {
			if (($field_update != "" && array_key_exists($field_update, $column)) || $req_update['field_action'][$key] == "add") {
				...
				if ($req_update['field_action'][$key] == "add") {
					// nuova colonna!
					$row_table[$field_update] = (object)['action'=>__(sprintf('Add new column: %s', $req_update['field_name'][$key]), 'database_press'),  'curr_action' =>'NEW',  'fields_errors'=>[]];
		...
		foreach ($row_table as $cfield => &$ctype) {
			...
			if ($new_primary_name != $primary_name) {
				$ctype->query_msg = __('If you change the primary key I cannot check if there will be any data loss', 'database_press');
			} else {
	...
	public function dbp_create_table_structure() {
		...
		if ($row_table->query_result) {
			dbp_fn::update_dbp_option_table_status($model_structure->get_table_name(), 'DRAFT', $_REQUEST['options']['description']);
		} else {
			$result['table_link'] = "";
			$result['ok'] = "no";
		}
