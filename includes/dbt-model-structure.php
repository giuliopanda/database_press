...
class Dbt_model_structure
{
    ...
    public function insert_column($name, $type, $length = 0,  $default ="", $primary_key=false, $attributes=false, $auto_increment= false, $null=false,   $comment="") {
        $sql_string = [];
        $name = substr(Dbt_fn_structure::clean_column_name($name),0 , 64);
        if ($name == "") {
            return false;
        }
        if (in_array($name, $this->name_list)) {
            $this->last_error = __("Duplicated column name!", 'database_tables');
            return false;
        }
        $this->name_list[] = $name;
        ...
        if ($primary_key) {
            if ( $this->primary_key != "") {
                $this->last_error = __('Only one primary key field is allowed', 'database_tables');
            } else {
        ...
        if (!$auto_increment) {
            if ($default != "") {
                $sql_string[] = "DEFAULT '".esc_sql(stripslashes($default))."'";
            } else if (!$null) {
                if (in_array(strtoupper($type), ['INT','FLOAT','BIGINT','MEDIUMINT','TINYINT','DECIMAL'])) {
    ...
    */
    public function get_sql_add_column($columns, $req_update, $key_req, $position = "") {
        ...
        $req_update['field_name'][$key_req]  = Dbt_fn_structure::clean_column_name($req_update['field_name'][$key_req]);
    ...
    */
    public function insert_new_column($column_name, $type, $length,  $null = false, $default = false, $attributes = '') {
        ...
        $column_name  = Dbt_fn_structure::clean_column_name($column_name);
        $this->get_structure();
        $columns = [];
    ...
    */
        ...
        if ($default !== false) {
            if ($primary != 't') {
                if ($default != "" && $default != "f") {
                    $change_sql[] = "DEFAULT '".esc_sql(stripslashes($default))."'";
                } else if ($default == "f") {
    ...
    public function get_create_sql() {
        ...
        if (count($this->columns) == 0) {
            $this->last_error = __("There are no columns to insert", 'database_tables');
            return "";
        } 
        if (count( $this->key) > 0) {
    ...
    public function change_unique_table_name($table_name = "", $check_with_prefix = true) {
		...
		$table_name = Dbt_fn_structure::clean_column_name($table_name);
    ...
    public function alter_index( $columns, $original_name, $name = "", $original_index = "INDEX", $type_of_index = "INDEX") {
        ...
        foreach ($columns as $col) {
            $column[] = '`'.esc_sql($col).'`';
            $new_name_array[] = Dbt_fn::clean_string($col);
        }
        if ($name == "") {
