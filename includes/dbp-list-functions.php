...
class  Dbp_functions_list {
    ...
    static function get_list_structure_config($model_items, $list_setting) {
        ...
        $count = 799;
        /**
         * @var dbpDs_list_setting[] $temp_items
         */
        $temp_items = [];
        dbp_functions_list::private_add_name_request($temp_items_2);
        if (is_array($temp_items_2)) {
            foreach ($temp_items_2 as $key=>$item) {
                $temp_list_structue = new DbpDs_list_setting();
                foreach ($item['schema'] as $ki =>$vi) {
                    $temp_list_structue->$ki = $vi;
                }
                $count++;
                $temp_list_structue->type = dbp_fn::h_type2txt($item['schema']->type); 
                $temp_list_structue->title = $item['schema']->name;
                $temp_list_structue->toggle = 'SHOW';
                $temp_list_structue->view = dbp_fn::h_type2txt($item['schema']->type);
                ...
                $temp_list_structue->mysql_name = dbp_functions_list::private_get_mysql_name($item['schema']);
        ...
        foreach ($list_setting as $key => $column_setting) {
            if (array_key_exists($key, $temp_items)) {
                ...
                $items[$key] = (new DbpDs_list_setting())->set_from_array(['name'=>$key,'type'=>'CUSTOM', 'origin'=>'CUSTOM', 'mysql_name'=>'', 'mysql_table'=>'', 'name_request'=>'']);
    ...
    static function convert_post_content_to_list_params($post_content = []) {
        ...
        if (!array_key_exists('list_setting', $content)) {
            ...
            if (is_array($content['list_setting'])) {
                foreach ($content['list_setting'] as $key=>$list ) {
                    $new_list_setting[$key] = (new DbpDs_list_setting())->set_from_array($list);
                }
            }
        ...
        if (!array_key_exists('form', $content)) {
            ...
            //@TODO converto $content['form'] in dbp_form_setting
        }
        ...
        if (array_key_exists('delete_params', $content)) {
            if (!is_a($content['delete_params'], 'DatabasePress\dbpDs_list_delete_params') ) {
     ...
     * @param Array $item La prima riga dei risultati di una query di database_press_model_base NON Convertiti in update_items_with_setting
     */
     ...
     * @return \DatabasePress\dbp_model
     */
        ...
        if (isset($sql_filter) && is_array($sql_filter)) {
            ...
            foreach ($sql_filter as $filter) {
                if (isset($filter['value'])) {
                    ...
                    $shortcode_request = array_merge($shortcode_request, dbp_functions_list::get_pinacode_params($filter['value'],'[%request'));
                    $shortcode_data = array_merge($shortcode_data, dbp_functions_list::get_pinacode_params($filter['value'],'[%data'));
    ...
    static function add_frontend_request_filter_to_model(&$table_model, $post_content, $list_id, $prefix = "") {
        ...
        $table_limit_start 		= dbp_fn::get_request_limit_start( $request_path .'_page', 1, ceil( $table_model->total_items / $table_limit )) ;
        
        $limit_start = ($table_limit_start -1) * $table_limit;
        $table_model->list_add_limit($limit_start, $table_limit);
       
        // order
        $table_sort = dbp_fn::get_request($request_path . '_sort', false); 

        if ($table_sort) {
            $sorts = explode(".", $table_sort);
            if (count($sorts) > 1) {
                $table_sort_order = array_pop($sorts);
                $table_sort_field =  dbp_fn::get_val_from_head_column_name($list_settings, implode(".",$sorts), 'mysql_name' );
                if ($table_sort_field != "") {
        ...
        $search = stripslashes(dbp_fn::get_request($request_path . '_search', false)); 
        if ($search) {
        ...
        foreach ($_REQUEST as $req => $req_val) {
            if (substr($req,0, strlen($request_path)) == $request_path && $req != $request_path . '_search') {
                ...
                $filter_temp =  dbp_fn::convert_head_column_in_filter_array($list_settings, $request_field, $req_val);
    ...
    public static function get_pinacode_params($string, $shortcode='[%params') {
        ...
        do {
           ...
           // print "<p>".$string." sh: ".$shortcode. " =  ".$find ."</p>";
            if ($find !== false) {
                ...
                //print "<p>TROVATO start:".$start ." LENGTH: ". $length. "=  (".$param .")</p>";
     ...
     * @param [type] $dbp_id
     * @return void
     * @deprecated Da eliminare, non si usa più
     */
    public static function process_saving_data_using_form_list($request_edit_table, $dbp_id) {
        $form_dbp_id = new Dbp_class_form($dbp_id);
        list($form_settings, $__) = $form_dbp_id->get_form();
        $error = "";
        foreach ($request_edit_table as $request_key => $form_value) {
			...
			foreach ($form_value as $table=>$rows) {
                if (isset($form_value[$table]["_dbp_alias_table_"]))  {
                    foreach ($form_value[$table]["_dbp_alias_table_"] as $key=>$alias_table) {
                       // print " Alias table: ".$alias_table."   \r\n";
                       
                        foreach ($rows as $field=>$value) {
                            if ($field == "_dbp_alias_table_ ") continue;
                            if (is_countable($value[$key])) {
								$value[$key] = maybe_serialize($value[$key]);
							}
                            $value[$key] = stripslashes( $value[$key] );
                            PinaCode::set_var(dbp_fn::clean_string($alias_table).".".dbp_fn::clean_string( $field), $value[$key]);
            ...
            */
            ...
            foreach ($form_value as $table=>$rows) {
                if (isset($form_value[$table]["_dbp_alias_table_"]))  {
                    foreach ($form_value[$table]["_dbp_alias_table_"] as $key=>$alias_table) {
                        foreach ($rows as $field=>$value) {
                            $field_setting = $form_dbp_id->find_setting_row_from_table_field($form_settings, $rows["_dbp_alias_table_"][$key], $field);
                            // TODO APPLY FILTER! 
                            ...
                            if ($field_setting != false && $field_setting->form_type == "CALCULATED_FIELD") {
                                ...
                                PinaCode::set_var(dbp_fn::clean_string($alias_table).".".dbp_fn::clean_string($field), $request_edit_table[$request_key][$table][$field]);
                            }
                            if ($field_setting != false && $field_setting->form_type == "UPLOAD_FIELD") {
                                ...
                                if ($new_file['name'] != "") {
                                    ...
                                    if( isset( $file_return['error'] ) || isset( $file_return['upload_error_handler'] ) || is_wp_error($file_return) ) {
                                        $error = _e('Failed to upload document to server', 'database_press');
                                    } else { 
    ...
    static function form_categ_tree($catId, $depth, $count_field, $selected_cats){
        ...
        if(count($categories) > 0) {
            foreach ($categories as $category) {
                ...
                $output .=  '<label class="dbp-form-cat dbp-form-cat-' . $depth .'"><input type="checkbox" name="fields_post_cats['.$count_field.'][]" value="'.$category->cat_ID.'"' . $checked . '>'. $category->cat_name . '</label>';
    ...
    static function form_user_roles( $count_field, $selected_roles){
        ...
        foreach ($roles as $key=>$rl) {
            ...
            $output .=  '<label class="dbp-form-cat "><input type="checkbox" name="fields_user_roles['.$count_field.'][]" value="'.$key.'"' . $checked . '>'. $rl['name'] . '</label>';
        }
        return $output;
    }
    ...
    private static function private_add_name_request(&$temp_items_2) {
        ...
        foreach ($temp_items_2 as &$item_o) {
            ...
            if ($item->name != "") {
                ...
                if (count($temp_exp) == 1) {
                    $name1 = dbp_functions_list::private_clean(6,  $item->name );
                } else {
                    $first =  array_shift($temp_exp);
                    $name1 = dbp_functions_list::private_clean(4, implode("", $temp_exp), $first );
                }
                $name2 = dbp_functions_list::private_clean(15, $item->name);
                $name3 = dbp_functions_list::private_clean(4,  $item->name, $item->table);
                $name4 = dbp_functions_list::private_clean(8, $item->name, $item->table);
            ...
            if (in_array( $calculate_name, $names) || $calculate_name == "") {
                $calculate_name =  dbp_fn::clean_string(dbp_fn::get_uniqid());
            } 
    ...
    private static function private_clean($substr, $var1, $var2 = "") {
        $var1 = substr(dbp_fn::clean_string($var1), 0, $substr);
        if ($var2 != "") {
            $var2 = substr(dbp_fn::clean_string($var2), 0, $substr);
            return $var2."_".$var1;
        }
        return $var1;
    }
    ...
    static public function execute_query_savedata($query_to_execute, $dbp_id = 0, $origin = "", $use_wp_fn = true) {
		...
		if (count($query_to_execute) > 0) {
			if ($dbp_id > 0) {
				$query_to_execute = apply_filters('dbp_save_data', $query_to_execute, $dbp_id, $origin);
			}
			foreach ($query_to_execute as $qtx) {
                if ($dbp_id > 0 && isset($qtx['setting'])) {
                    foreach ($qtx['setting'] as $setting) {
                        if ( $setting->form_type == "CALCULATED_FIELD") {
                            ...
                            if ( $setting->custom_value_calc_when == "EVERY_TIME" || ($setting->custom_value_calc_when == "EMPTY" && $qtx['sql_to_save'][$setting->name] == "" || empty($qtx['sql_to_save'][$setting->name]))) {
                        ...
                        if ( $setting->default_value != "" &&  !isset($qtx['sql_to_save'][$setting->name]) && $setting->name !=  "_dbp_alias_table_") {
    ...
    static function add_lookups_column(&$table_model, $post) {
        ...
        foreach ($post->post_content['list_setting'] as $key=>$setting) {
            ...
            if ($setting->view == "LOOKUP") {
                ...
                $alias = Dbp_fn::clean_string("lookup_".$key);
                ...
                foreach ($setting->lookup_sel_txt as $new_field) {
                    $new_alias = $key."_".Dbp_fn::clean_string($new_field);
