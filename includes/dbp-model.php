...
class  Dbp_model {
    ...
    public function get_list($check_same_column = true) {
        ...
        if ($this->sql_type() == "select" || $this->sql_type() == "show") {
            ...
            if ($draft_is_ok && $close_is_ok) {
                ...
                if (!$draft_is_ok) {
                    $this->last_error = __('This type of query is not allowed because the table is in PUBLISH state', 'database_press');
                } else {
                    $this->last_error = __('This type of query is not allowed because the table is in CLOSE state', 'database_press');
                }
                $this->effected_row = 0;
                $result = [['effected_row' =>['schema'=>(object)['name'=>'effected_row','orgname'=>'','orgtable'=>'','table'=>'','type'=>'','db'=>'']]], ['effected_row'=>0]];
            }
    ...
    public function update_items_with_setting($post = false, $htmlentities = true, $text_length = 0) {
        $dbp_id = 0;
        $post_content = dbp_functions_list::convert_post_content_to_list_params();
        if ($post != false) {
            $post_content = $post->post_content;
            $dbp_id = $post->ID;
        } 
        if (is_array($post_content) && isset($post_content['list_setting'])) {
            $setting_custom_list =  dbp_functions_list::get_list_structure_config($this->items, $post_content['list_setting']);
        } else {
        ...
        $this->items = $setting->execute_list_settings($this, $setting_custom_list, $list_general_setting, $dbp_id);
    }
    ...
    private function check_is_ok_for_draft($sql) {
        ...
        if (stripos($sql, 'DROP ') !== false && stripos($sql, 'DATABASE ') !== false)  {
            $this->last_error = __('You cannot remove the entire database','database_press');
            return false;
        }
        ...
        if ((stripos($sql, 'ALTER ') !== false && stripos($sql, 'TABLE ') !== false) || (stripos($sql, 'DROP ') !== false && stripos($sql, 'TABLE ') !== false) || (stripos($sql, 'TRUNCATE ') !== false && stripos($sql, 'TABLE ') !== false)) {
            ...
            if ($info['status'] == 'DRAFT') {
                ...
                $this->last_error = __('Only tables in draft can be Altered. Go to structure and change the status of the table','database_press');
            }
        }
    ...
    private function get_list_check_same_column($list_last_get_col_info) {
        ...
        foreach ($list_last_get_col_info as $val ) {
            if (in_array($val->name, $temp_names)) {
                $this->last_error = sprintf(__('The query has multiple columns (<b>%s</b>) with the same name.', 'database_press'), $val->name);
                $this->effected_row = 0;
                $this->items = [];
