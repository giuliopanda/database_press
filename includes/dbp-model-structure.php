...
class  Dbp_model_structure
{
    ...
    public function insert_column($name, $type, $length = 0,  $default ="", $primary_key=false, $attributes=false, $auto_increment= false, $null=false,   $comment="") {
        ...
        if (in_array($name, $this->name_list)) {
            $this->last_error = __("Duplicated column name!", 'database_press');
            return false;
        }
        $this->name_list[] = $name;
        ...
        if ($primary_key) {
            if ( $this->primary_key != "") {
                $this->last_error = __('Only one primary key field is allowed', 'database_press');
            } else {
    ...
    */
    ...
    public function get_create_sql() {
        ...
        if (count($this->columns) == 0) {
            $this->last_error = __("There are no columns to insert", 'database_press');
            return "";
        } 
        if (count( $this->key) > 0) {
        ...
        if ( $this->primary_key != "") {
            ...
            $this->last_error = __("Primary key is missing", 'database_press');
        }
        if (!$this->check_table_name()) {
            $this->last_error = sprintf(__("Table %s already exists!", 'database_press'),  $this->get_table_name());
            return false;
        }
