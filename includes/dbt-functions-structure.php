...
class Dbt_fn_structure {
    ...
    static function convert_show_column_mysql_row_to_form_data($column) {
        ...
        list($new_column->field_type, $new_column->field_length, $new_column->attributes) = Dbt_model_structure::reverse_type($column->Type);
        if ($column->Null == 'NO') {
            $new_column->null = "f";
        } else {
            $new_column->null = "t";
        ...
        if ($column->Key == "PRI" && $column->Extra == "auto_increment") {
            $new_column->dbt_primary = "t";
        } else {
            $new_column->dbt_primary = "f";
        }
    ...
    static function create_temporaly_table_from($table) {
        global $wpdb;
        $table_temp = substr(Dbt_fn::clean_string($table),0,56)."__ctemp";
    ...
    public static function get_preset($new_column) {
        ...
        if ($new_column->field_type == "DOUBLE" && $new_column->field_length == "11,2" && $new_column->attributes == "" && $new_column->null == "f"  && $new_column->default == "") {
            return "double";
        }
