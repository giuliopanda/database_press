...
class  Dbp_loader_import {
	...
	public function dbp_import_csv_table_structure() {
		...
		if (array_key_exists('table', $_REQUEST) && array_key_exists('elid', $_REQUEST)) {
			$columns = dbp_fn::get_table_structure(@$_REQUEST['table']);
			$ris['elid'] = $_REQUEST['elid'];
			$csv_columns = $this->prepare_columns();
			ob_start();
			$unique =  dbp_fn::clean_string($_REQUEST['table'])."_".$_REQUEST['elid'];
			?>
			...
			<table class="wp-list-table widefat striped dbp-table-view-list dbp-table-preview-csv">	
				<thead>
					<tr>
						<th class="dbp-table-th dbp-th-dim-gen"><?php echo _e('Table Column','database_press'); ?></td>
						<th class="dbp-table-th dbp-th-dim-gen"><?php echo _e('Type','database_press'); ?></td>
						<th class="dbp-table-th dbp-th-dim-gen"><?php echo _e('CSV column','database_press'); ?></td>
					</tr>
				</thead>
				<tbody>
				<?php 
				...
				foreach ($columns as $c) {
					...
					?>
						<tr>
							...
							<td><?php dbp_fn::html_select($csv_columns, true, 'class="js-fields-choosen'.$key_class.'" name="import_csv_column['.esc_attr($_REQUEST['elid']).'][]" style="min-width:400px"',  $column_search); ?></td>
						</tr>
				<?php 
				}
				?>
	...
	public function dbp_test_import_csv_data() {
		...
		if (!is_array($import_tables) || count($import_tables) == 0 ) {
			wp_send_json(['result'=>'no','msg'=>__('You must select at least one table before you can import data','database_press')]);
			die;
		}
		...
		$table_insert  = dbp_fn_import::convert_csv_data_request_to_vars($import_tables, $_REQUEST['import_field'], $_REQUEST['import_csv_column']);
		...
		foreach ($table_insert as $uniqid_key => $ti) {
			...
			if ($table_temp) {
				...
				ob_start();
				?>
				<div class="dbp-alert-info"><?php _e(sprintf('I start testing the import for the "<b>%s</b>" table', $ti->table), 'database_press'); ?></div>
				<?php 
				if ($primary_key == "") : ?>
					<div class="dbp-alert-sql-error"><?php _e(sprintf('The table "<b>%s</b>" must have a field set auto increment primary key', $ti->table ),'database_press'); ?></div>
				<?php elseif (count($results) == 0) : ?>
					<div class="dbp-alert-sql-error"><?php _e('I have not found any data to insert','database_press'); ?></div>
				<?php else : ?>
						...
						foreach ($results as $r) {
							...
							foreach ($columns as $c) {
								...
								if (isset($r['data'][$field])) {
									...
									if (isset($r['row']->$field) && $current_data != $r['row']->$field) {
										$icon = '<span class="dashicons dashicons-info dbp-icon-alert" title="'.esc_attr(__('The value is different from the original csv','database_press')).'"></span>';
									}
								}
							...
							if ($r['error'] != "") {
								if (strtoupper(substr($r['sql'],0, 6)) == "INSERT" || strtoupper(substr($r['sql'],0, 6)) == "UPDATE" ) {
									...
									_e('The query failed','database_press');
								}
							} else if ($r['result']  === false) {
								if (strtoupper(substr($r['sql'],0, 6)) == "INSERT" || strtoupper(substr($r['sql'],0, 6)) == "UPDATE" ) {
									echo "<b>".$r['sql']."</b><br>".__('The query failed','database_press');
								} else {
									_e('The query failed','database_press');
								}
							}
				...
				$html_result[] = ['html'=>'<div class="dbp-alert-info">' . $ti->table . '</div><div class="dbp-alert-sql-error">'.__('The insertions cannot be performed for the primary key in the table','database_press').'</div>', 'table'=>$ti->table, 'uniqid'=>$uniqid_key];
			}
		}
	...
	public function dbp_import_csv_data() {
		...
		foreach ($table_insert as $uniqid_key => $ti) {
			...
			if ($primary_key && $primary_key != "" ) {
					...
					if (count($array_insert) > 0) {
						...
						if ($temp['error'] != "" || $temp['result'] === false) {
							...
							if ($temp['error'] != "") {
								...
								$csv_items[$key_csv_items][$name_new_column_csv]  = __('The query failed: '.$temp['sql'], 'database_press');
							}
							*/
							$response['errors']++;
							//break;
				...
				$response['last_error'] = __('The insertions cannot be performed for the primary key in the table','database_press');
			}
		}
	...
	private function prepare_columns() {
		...
		$csv_columns = [''=>__('Don\'t insert','database_press')];
		foreach ($csv_item as $key=>$cc) {
			if ($key != "") {
				$csv_columns[$key] = $cc;
			}
		} 
		$csv_columns['__[custom]__'] = __('[Custom Text]','database_press');
		return $csv_columns;
	}
	/**
	 * Prepara la lista di colonne da inserire nel select in cui configurare quali colonne del csv vengono associati a quali campi del db
