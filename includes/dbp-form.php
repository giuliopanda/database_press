...
class  Dbp_class_form {
	...
	public function __construct($param) 
	{
		if (is_numeric($param)) {
			$this->dbp_id = $param;
			$post  = dbp_functions_list::get_post_dbp($this->dbp_id);
	...
	public function save_data($values, $use_wp_fn = true) {
		...
		$ris =  dbp_functions_list::execute_query_savedata($query_to_execute, $this->dbp_id, 'php-form', $use_wp_fn);
		$execute = true;
		foreach ($ris as $r) {
	...
	private function convert_table_to_group($schemas) {
		...
		foreach ($temp_groups as $key=>$group) {
			...
			foreach($group as $k=>$g) {
				if (!isset($group_unique[$g->orgtable."".$g->orgname])) {
					$group_unique[$g->orgtable."".$g->orgname] = 1;
					$new_group[$k] = $g;
				}
			}
		//if ($key != '__orphan__') {
	...
	private function convert_post_content_form_to_table_options_single($post_content, &$group) {
		...
		$primary_key = dbp_fn::get_primary_key($group->orgtable);
	...
	private function convert_post_content_form_to_setting_single($post_content, &$field) {
		...
		if (isset($form_field['options'])) {
			$field->options = dbp_fn::parse_csv_options($form_field['options']);
		}
		...
		if (isset($form_field['form_type'])) {
			...
			if ( $field->form_type == "LOOKUP") {
				...
				$field->lookup_where = $form_field['lookup_where'];
			}
						...
						if (property_exists($item, $field)) {
							if ($item->$field == null) {
								...
								if ($setting_field->form_type == "LOOKUP") {
									if ($item->$field > 0) {
										$table_model = new Dbp_model($setting_field->lookup_id);
