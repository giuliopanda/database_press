...
class Dbt_model {
    ...
    /**
     * @var Dbt_util_marks_parentheses $ulitities_marks La classe che converte " e parentesi della query per poi renderla più semplice da elaborare
     */
     ...
     * Viene compilato solo da Dbt_fn::items_add_action
     */
    ...
    public function __construct($table_name = "") {
        ...
        $this->ulitities_marks = new Dbt_util_marks_parentheses();
        $this->prepare();
	}
     ...
     * @param String $query
     */
    public function prepare($query = "", $stripslashes = true) {
        if(trim($query) != "") {
            $query = Dbt_fn::all_trim($query);
            while  (substr($query,-1, 1) == ";") {
                $query = substr($query, 0, -1);
            }
            if ( $stripslashes ) {
                $this->current_query = trim(stripslashes($query));
            } else {
                $this->current_query = trim($query);
            }
        } else if ($this->table_name != "") {
            $this->current_query = 'SELECT * FROM  `'. esc_sql($this->table_name).'`';
        } else {
            return false;
        }
            ...
            foreach ($list_of_parts as $single_from) {
               // print "<p>".$single_from."</p>";
                ...
                if (count($from_ris_2) == 2) {
                    ...
                    $form_2 = Dbt_fn::all_trim($this->ulitities_marks->restore(array_shift($from_ris_2)));
                } else {
                    $left = str_ireplace([' as ', '  '], ' ', Dbt_fn::all_trim($single));
                    $form_2 = '';
                }
                $left_2 = explode(" ", $left);
                ...
                if (count ($left_2) == 1) {
                    $table = Dbt_fn::all_trim(str_replace("`","", $this->ulitities_marks->restore(reset($left_2))));
                    $table_alias = trim(str_replace("`","", $this->ulitities_marks->restore(reset($left_2))));
                    $from_split[] = [$table, $table_alias, '',  Dbt_fn::all_trim($this->ulitities_marks->restore($single_from))] ;
                }
                if (count ($left_2) == 2) {
                    $table = Dbt_fn::all_trim(str_replace("`","", $this->ulitities_marks->restore(array_shift($left_2))));
                    $table_alias = trim(str_replace("`","", $this->ulitities_marks->restore(array_shift($left_2))));
                    $from_split[] = [$table, $table_alias, $form_2,   Dbt_fn::all_trim($this->ulitities_marks->restore($single_from))];
                }
        ...
        if ($this->sql_type() == "select") {
            ...
            if ($limit > 0) {
                ...
                if (count($limit_temp) == 1) {
                    $limit_start = 0;
                    $limit_end = intval(Dbt_fn::all_trim(array_shift($limit_temp)));
                }
                if (count($limit_temp) == 2) {
                    $limit_start = intval(Dbt_fn::all_trim(array_shift($limit_temp)));
                    $limit_end = intval(Dbt_fn::all_trim(array_shift($limit_temp)));
                }
            } 
        }
    ...
    public function list_add_order($field, $order) {
        if($this->sql_type() == "select") {
            ...
            $sql = Dbt_fn::all_trim($sql);
            $this->current_query = $sql." ORDER BY ".$field." ".$order;
            if ($limit_sql != "") {
     ...
     * @return Array [{schema:{}},{item},{item}] La prima riga da le informazioni sulla colonna!
 	 */
    public function get_list($check_same_column = true) {
        global $wpdb;
        ...
        if ($this->sql_type() == "select" || $this->sql_type() == "show") {
            ...
            if (is_wp_error($result) || !empty($wpdb->last_error)) {
             ...
             //   var_dump ($list_last_get_col_info);
            ...
            if ($draft_is_ok && $close_is_ok) {
                ...
                if (!$draft_is_ok) {
                    $this->last_error = __('This type of query is not allowed because the table is in PUBLISH state', 'database_tables');
                } else {
                    $this->last_error = __('This type of query is not allowed because the table is in CLOSE state', 'database_tables');
                }
                $this->effected_row = 0;
                $result = [['effected_row' =>['schema'=>(object)['name'=>'effected_row','orgname'=>'','orgtable'=>'','table'=>'','type'=>'','db'=>'']]], ['effected_row'=>0]];
            }
     ...
     * 
     * @return Array [{schema:{}},{item},{item}] La prima riga da le informazioni sulla colonna!
     * @deprecated Passato a includes\dbt-model-structure.php
 	 */
        ...
        if (is_array($this->items) && count($this->items) > 0) {
            ...
            foreach ($table_header as $k=>$th) {
                if (isset($th->table) && $th->original_table != "" && !isset($tables[$th->original_table])) {
                    $tables[$th->original_table] = Dbt_fn::get_dbt_option_table($th->original_table);
                    if ($table_status == '') {
        ...
        if($this->sql_type() == "select") {
            ...
            if ($pri == "") {
                ...
                if (strpos($column, '`.`') !== false) {
                    ...
                    $pri_alias = '`'.esc_sql($table_alias).'`.`'.esc_sql($pri).'`';
                } else {
                    $pri_alias = '`'.esc_sql($this->table_name).'`.`'.esc_sql($pri).'`';
                }
                $new_fragment = ' '.esc_sql($column).' AS c, ' . $pri_alias . ' as p, count('. $pri_alias .') as n ';
            }
            ...
            $group_position = strripos($sql, ' GROUP '); // trovo l'ultima occorrenza
            if ($group_position > 0) {
    ...
    public function get_pirmaries($alias = false) {
        ...
        if (!$alias) {
            foreach ($tables as $table) {
                ...
                $pri = Dbt_fn::get_primary_key($orgtable);
                if ($pri != "") {
        ...
        if (count($exp) == 2) {
            $field_temp = explode(".", $exp[0]);
                if (count($field_temp) == 2) {
                    ...
                    $value =  Dbt_fn::all_trim($this->ulitities_marks->restore( $exp[1] ));
                if (substr($value,0,1) == "'" || substr($value,0,1) == '"') {
                    return [$table, $field, substr($value,1,-1)];
                } else {
                    ...
                    if (count($value_temp) == 2) {
                        ...
                        if ($base_table == $table) {
                            return [$table, $field, "[%".Dbt_fn::clean_string($value_table).".".Dbt_fn::clean_string($value_field)."]", $value_table,  $value_field];
                        } else if ($base_table == $value_table) {
                            return [$value_table, $value_field, "[%".Dbt_fn::clean_string($table).".".Dbt_fn::clean_string($field)."]", $table, $field];
                        }
                    }
                }
            }
    ...
    private function  get_primary_key($table = "") {
        ...
        return Dbt_fn::get_primary_key($table);
    }
            ...
            foreach ($filter as $f) {
                ...
                if (is_string($f['value'])) {
                   ...
                   // print (PinaCode::execute_shortcode($f['value']));
                    ...
                    if (is_string($f['value']) && trim($f['value']) == "") continue;
                ...
                switch ($f['op']) {
                    ...
                    case "BETWEEN":
                    case "NOT BETWEEN":
                        $val = Dbt_fn::set_sql_where_between_value($f['column'], $f['value'], $f['op']);
                        if ($val != "") {
                            $array_where[] = $val;
                        }  
                        break;
                    ...
                    case "NOT IN":
                        ...
                        if (count ($new_ids) > 0 && strpos($f['column'],'`.`') !== -1) {
                            $array_or[] = "`".$table ."`.`".$pri."` ".$op." (".implode(", ",$new_ids).")";
                        }
                        if (count($array_or) == 1) {
    ...
    private function check_is_ok_for_draft($sql) {
        ...
        if (stripos($sql, 'DROP ') !== false && stripos($sql, 'DATABASE ') !== false)  {
            $this->last_error = __('You cannot remove the entire database','database_tables');
            return false;
        }
        ...
        if ((stripos($sql, 'ALTER ') !== false && stripos($sql, 'TABLE ') !== false) || (stripos($sql, 'DROP ') !== false && stripos($sql, 'TABLE ') !== false) || (stripos($sql, 'TRUNCATE ') !== false && stripos($sql, 'TABLE ') !== false)) {
            ...
            $info = Dbt_fn::get_dbt_option_table($alter_table);
            if ($info['status'] == 'DRAFT') {
                $is_ok_for_draft = true;
            } else {
                $this->last_error = __('Only tables in draft can be Altered. Go to structure and change the status of the table','database_tables');
            }
        }
    ...
    private function check_is_ok_for_close($sql) {
        ...
        $info = Dbt_fn::get_dbt_option_table(array_shift($temp_sql));
