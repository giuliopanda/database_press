<?php
/**
 * Classe che gestisce i file temporanei di database_press
 * 
 */
...
class  Dbp_temporaly_files
{
    ...
    public function get_dir() {
        ...
        $this->last_error = sprintf(__("I cannot access a temporaly folder: %s", 'database_press'), $dir);
        return false;
    }
    ...
    public function read($filename, $maybe_unserialize = true) {
        ...
        if (!is_file($dir.$filename)) {
            $this->last_error = __("Temporary file cannot be read", 'database_press');
            return false;
        }
    ...
    public function delete($filename = "") {
        ...
        if (!is_file($dir.$filename)) {
            ...
            unlink ($dir.$filename);
            if (is_file($dir.$filename)) {
                $this->last_error = sprintf(__("I can't delete the file: %", 'database_press'), $dir.$filename);
                return false;
            }
            return true;
        }
    ...
    public function move_uploaded_file($post_file_name) {
        if (!isset($_FILES[$post_file_name]['tmp_name']) || $_FILES[$post_file_name]['tmp_name'] == "") {
            $this->last_error = __('No file uploaded', 'database_press');
            return false;
        }
        ...
        if ($ris == false) {
            $this->last_error = __('Move uploaded file error', 'database_press');
            return '';
        }
    ...
    public function read_csv($filename, $delimiter = "", $first_row_key = true, $row_read = 0) {
        ...
        $keys_column = [];
        if (!is_file($dir.$filename)) {
            $this->last_error = __('The file you are trying to upload does not exist', 'database_press');
            return false;
        }
        if (($handle = fopen($dir.$filename, "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 262144, $delimiter)) !== FALSE) {
                ...
                if ($first_row_key && $count_row == 1) {   
                    ...
                    $keys_column = [];
                    foreach ($data as $k=>$v) {
                        $keys_column[$k] = __('Column ','database_press').$k;
                    }
                    $result[] = $keys_column;
                    $result[] = $data;
                }
