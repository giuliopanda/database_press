...
class Dbt_temporaly_files
{
    ...
    public function read_csv($filename, $delimiter = "", $first_row_key = true, $row_read = 0) {
        ...
        if ($delimiter == "") {
            ...
            $delimiter = Dbt_fn::convert_special_to_char($delimiter);
        }
        $result = [];
        $count_row = 0;
        $keys_column = [];
        ...
        if (($handle = fopen($dir.$filename, "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 262144, $delimiter)) !== FALSE) {
                ...
                if ($first_row_key && $count_row == 1) {   
                    ...
                    $keys_column = [];
                    foreach ($data as $k=>$v) {
                        $keys_column[$k] = __('Column ','database_tables').$k;
                    }
                    $result[] = $keys_column;
                    $result[] = $data;
                }
    ...
    public function csv_structure($filename, $delimiter = "", $first_row_key = true) {
        ...
        foreach ($result as &$r) {
            $r->field_name = Dbt_fn::clean_string(strtolower(trim($r->name)));
            $r->field_length = "";
            $r->primary = "f";
            $r->attributes = "";
            $r->ai = "f";
            ...
            $r->preset = Dbt_fn_structure::get_preset($r);
        }
       // var_dump ($result);
        return $result;
    }
