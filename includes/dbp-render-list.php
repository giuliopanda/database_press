...
class  Dbp_render_list {
	...
	/**
	 * @var \DatabasePress\dbp_search_form $search
	 */
	var $search = false;
	...
	function __construct($post_id, $mode = null, $prefix = "") {
        ...
        $this->table_model = dbp_functions_list::get_model_from_list_params($post->post_content);
		...
		if ($this->table_model) {
            ...
            dbp_functions_list::add_frontend_request_filter_to_model($this->table_model, $post->post_content , $post_id, $prefix);
            Dbp_functions_list::add_lookups_column($this->table_model, $post);

			$this->table_model->get_list();
			$total_row = $this->table_model->get_count();
			PinaCode::set_var('total_row', absint($total_row));
			$this->uniqid_div = 'dbp_' . dbp_fn::get_uniqid();
	...
	public function get_frontend_view($key, $default = false) {
		if (array_key_exists($key, $this->frontend_view_setting)) {
			return \stripslashes($this->frontend_view_setting[$key]);
		} else {
			return $default;
		}
	}
	...
	public function table( $custom_class = "", $table_sort = null) {
		...
		<?php echo apply_filters('dbp_frontend_table_thead', ob_get_clean(), $this->list_id, $array_thead); ?>
		<tbody>
		<?php foreach ($items as $item) : ?>
			<tr>
				...
				foreach ($array_thead as $key=>$setting) { 
					...
					?><td class="dbp-table-td<?php echo $setting->width.' '.$formatting_class; ?> dbp-td-<?php echo strtolower($setting->align); ?>"><div class="btn-div-td "><?php 
						echo $item->$key;
					?></div></td> <?php
				} 
				?>
	...
	public function search($btn = true) {
		...
		$field_use = apply_filters( 'dbp_frontend_search', $field_use, $this->prefix_request."_search", $this->list_id);
		if ($field_use == "") return;
	...
	public function submit($label = '') {
		...
		<div class="dbp-search-row">
			<div class="dbp-search-button dbp-search-button-<?php echo $this->color; ?>" onclick="dbp_submit_simple_search(this)"><?php _e($label, 'database_press'); ?></div>
			<div class="dbp-search-button dbp-search-button-<?php echo $this->color; ?>" onclick="dbp_submit_clean_simple_search(this)"><?php _e('Clean', 'database_press'); ?></div>
		</div>
		<?php
	}
	...
	public function single_field_search($field_name,  $label = '') {
		...
		$field_name = apply_filters( 'dbp_frontend_search', $field_name, $this->prefix_request."_".$field_name_request, $this->list_id);
	...
	public function open_block($add_class = true) {
		if (!$this->block_opened) {
			...
			$this->search = new Dbp_search_form();
			if ($this->mode != 'link' ) {
			...
			if ($this->mode != 'link' ) {
				...
				?>
				<input type="hidden" class="js-dbp-sorting" name="<?php echo esc_attr($this->prefix_request); ?>_sort" value="<?php echo @$_REQUEST[$this->prefix_request."_sort"]; ?>">
