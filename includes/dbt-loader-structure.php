...
class Dbt_loader_structure {

	public function __construct() {
		// Questa una chiamata che deve rispondere un csv
		add_action( 'wp_ajax_dbt_update_table_structure_test', [$this, 'dbt_update_table_structure_test']);
		add_action( 'wp_ajax_dbt_update_table_structure', [$this, 'dbt_update_table_structure']);
		// creo una nuova tabella		
		add_action( 'wp_ajax_dbt_create_table_structure', [$this, 'dbt_create_table_structure']);		
	}
	 ...
	 */
	public function dbt_update_table_structure_test() {
		Dbt_fn::require_init();
		$result = ['result'=>'ok'];
		$model_structure = new Dbt_model_structure($_REQUEST['table']);
		...
		foreach ($model_structure->items as $cs) {
			...
			$column[$cs->Field] = Dbt_fn_structure::convert_show_column_mysql_row_to_form_data($cs);
		}
		$table_temp = Dbt_fn_structure::create_temporaly_table_from($_REQUEST['table']);
		$model_structure_temp = new Dbt_model_structure($table_temp);
		$pre_edit = Dbt_fn_structure::load_rows($table_temp, $primary_name);
		...
		foreach ($req_update['field_original_name'] as $key=>$field_update) {
			if (($field_update != "" && array_key_exists($field_update, $column)) || $req_update['field_action'][$key] == "add") {
				...
				if ($field_update != $req_update['field_name'][$key]) {
					$req_update['field_name'][$key] = Dbt_fn_structure::clean_column_name($req_update['field_name'][$key]);
				}
				if ($req_update['field_action'][$key] == "add") {
					// nuova colonna!
					$row_table[$field_update] = (object)['action'=>__(sprintf('Add new column: %s', $req_update['field_name'][$key]), 'database_tables'),  'curr_action' =>'NEW',  'fields_errors'=>[]];
				...
				if (count ($row_table[$field_update]->sql) > 0) {
					...
					$row_table[$field_update]->sql = str_replace($table_temp, $_REQUEST['table'], implode ("<br>", $row_table[$field_update]->sql));
				} else {
		...
		$post_edit = Dbt_fn_structure::load_rows($table_temp, $new_primary_name);
		foreach ($row_table as $cfield => &$ctype) {
			$ctype->count_pre = count($pre_edit);
			$ctype->fields_errors = [];
			if ($new_primary_name != $primary_name) {
				$ctype->query_msg = __('If you change the primary key I cannot check if there will be any data loss', 'database_tables');
			} else {
		...
		Dbt_fn_structure::drop_temporaly_table($table_temp);
	 ...
	 * $_REQUEST['table_update']; [field_original_name, field_action, field_name, field_type ,field_length, null, primary]
	 */
	public function dbt_update_table_structure() {
		Dbt_fn::require_init();
		$table = $_REQUEST['table'];
		$model_structure = new Dbt_model_structure($table);
		$table_link = add_query_arg(['section'=>'table-browse', 'table'=>$model_structure->get_table_name()], admin_url("admin.php?page=database_tables"));
		...
		if ($table_options['status'] == "DRAFT") {
			...
			if (isset($_REQUEST['structure_table_name']) && $_REQUEST['structure_table_name'] != $_REQUEST['table']) {
				$new_name = Dbt_fn_structure::clean_column_name($_REQUEST['structure_table_name']);
				$sql = 'RENAME TABLE `'.$_REQUEST['table'].'` TO `'.$new_name. '`;';
				$row_table_temp = (object)['action'=>'RENAME TABLE', 'curr_action' =>'renametable', 'sql'=>$sql, 'new_table'=>$new_name, 'old_table'=>$_REQUEST['table']];
	 ...
	 * $_REQUEST['table_update']; [field_original_name, field_action, field_name, field_type ,field_length, null, primary]
	 */
	public function dbt_create_table_structure() {
		Dbt_fn::require_init();
		// TODO manca l'update dei post
		
		$table = $_REQUEST['structure_table_name'];
		$model_structure = new Dbt_model_structure($table);
		$table_link = add_query_arg(['section'=>'table-browse', 'table'=>$model_structure->get_table_name()], admin_url("admin.php?page=database_tables"));
		$result = ['result'=>'ok','table_link'=>$table_link];
		$row_table = (object)[];
		$row_table->query_result = $model_structure->sql_create_table_row($_REQUEST['table_update']);
		// $_REQUEST[structure_table_name] = nome tabella
		// row: field_name, field_type, field_length, attributes, default, null primary (f|t)
		if ($row_table->query_result) {
			Dbt_fn::update_dbt_option_table_status($model_structure->get_table_name(), 'DRAFT', $_REQUEST['options']['description']);
		} else {
			$result['table_link'] = "";
			$result['ok'] = "no";
		}
		...
		$row_table->action =" CREATE TABLE `".$table."`";
		$row_table->sql  = "";
		$result['row_table'] = [$row_table];
		wp_send_json($result);
		die;
